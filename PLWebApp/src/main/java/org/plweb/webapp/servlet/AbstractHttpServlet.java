 ...
 */
package org.plweb.webapp.servlet;
 ...
 * {@link java.util.regex.Matcher} for details. The servlet init parameter names
 * are:
 * 
 * <pre>
 ...
 * <p/>
 * <p/>
 * This implementation provides a verbosity flag switching log statements. The
 * servlet init parameter name is:
 * 
 * <pre>
 ...
 * <p/>
 * <p/>
 * In order to support class-loading-troubles-debugging with Tomcat 4 or higher,
 * you can log the class loader responsible for loading some classes. See <a
 * href="http://jira.codehaus.org/browse/GROOVY-861">GROOVY-861</a> for details.
 ...
 * other servlet container using custom class loader setups, you can fallback to
 * use (slower) reflection in Groovy's MetaClass implementation. Please contact
 * the dev team with your problem! Thanks. The servlet init parameter name is:
 * 
 * <pre>
...
public abstract class AbstractHttpServlet extends HttpServlet implements
	...
	/**
	 * Either <code>null</code> or a compiled pattern matcher read from "
	 * {@code resource.name.regex}" and used in
	 * {@link AbstractHttpServlet#getScriptUri(HttpServletRequest)}.
	 */
	...
	/**
	 * Mirrors the static value of the reflection flag in MetaClass. See
	...
	public URLConnection getResourceConnection(String name)
		...
		try {
			...
			if (url == null) {
				...
				url = new URL("file", "",
						servletContext.getRealPath(tryScriptName));
			}
			return url.openConnection();
		} catch (IOException e) {
			throw new ResourceException("Problems getting resource named \""
					+ name + "\"!", e);
		}
	}
	...
	protected String getScriptUri(HttpServletRequest request) {
		/*
		 * Log some debug information for
		...
		if (logGROOVY861) {
			...
			log(" l.getClass()           :\""
					+ request.getClass().getClassLoader().getClass() + "\"");
			/*
		...
		//
		// Check to see if the requested script/template source file has been
		// the
		 ...
		 * http://cvs.groovy
		 * .codehaus.org/viewrep/groovy/groovy/groovy-core/src/main
		 * /groovy/servlet/GroovyServlet.java?r=1.10#l259
		 */
	...
	private String applyResourceNameMatcher(final String aUri) {
		...
		if (matcher != null) {
			...
			if (!uri.equals(replaced)) {
				if (verbose) {
					log("Replaced resource name \"" + uri + "\" with \""
							+ replaced + "\".");
				}
				uri = replaced;
			}
	...
	public void init(ServletConfig config) throws ServletException {
		...
		if (regex != null) {
			...
			this.resourceNameMatcher = Pattern.compile(regex, flags)
					.matcher("");
		...
		if (verbose) {
			...
			if (resourceNameMatcher != null) {
				log("resource.name.regex = "
						+ resourceNameMatcher.pattern().pattern());
			} else {
	 ...
	 * <ul>
	 * <li><tt>"forward(String path)"</tt> :
	 * request.getRequestDispatcher(path).forward(request, response);</li>
	 * <li><tt>"include(String path)"</tt> :
	 * request.getRequestDispatcher(path).include(request, response);</li>
	 * <li><tt>"redirect(String location)"</tt> :
	 * response.sendRedirect(location);</li>
	 * </ul>
	 * </p>
	 * <p/>
	 * <p>
