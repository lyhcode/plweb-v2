...
<form action="${response.encodeUrl('/login/login_auth.groovy')}" method="post" class="form-login"><div class="form-login-inner">
	...
	<input type="hidden" name="login_ajax_url" value="${response.encodeUrl('/login/login_ajax.groovy')}" />
	
	<div class="field">
		<span>電子郵件</span>
  		<input type="text" name="email" class="input-text" title="請輸入帳號" value="${helper.sess_fetch('login_email', '')}" />
	</div>
	
	<div class="field">
		<span>密碼</span>
  		<input type="password" name="password" class="input-text" title="請輸入密碼" value="" />
  	</div>
	
	<button class="fancy-button-01 button-login"><b>立即登入</b></button>
	
	<a href="#" class="button-signup">註冊新帳號</a>
	
	<noscript>
		<div class="login-error">
		<span>錯誤！</span>您的瀏覽器不支援JavaScript！無法繼續使用本系統。
		</div>
	</noscript>
