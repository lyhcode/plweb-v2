...
public class FrameTaskEditor extends JFrame implements ActionListener,
	...
	private JComponent createBasicComponent() {
		...
		p.setBorder(BorderFactory.createTitledBorder("Basic Settings"));
	...
	private JPanel createPropertiesPanel() {
		...
		for (String name : task.getProperties().keySet()) {
			String value = task.getProperties().get(name);
	...
	public void updateTask() {
		...
		for (int row = 0; row < tmProperties.getRowCount(); row++) {
			properties.put((String) tmProperties.getValueAt(row, 0),
					(String) tmProperties.getValueAt(row, 1));
		}
